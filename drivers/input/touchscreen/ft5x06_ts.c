...
#endif
	char gestures_enabled;
	char gestures_blocked;
};
...
{
	struct ft5x06_ts_data *data = (struct ft5x06_ts_data *) lparam;

	data->gestures_blocked = 0;
}
...
static irqreturn_t ft5x06_ts_interrupt(int irq, void *dev_id)
{
	...
	if(data->gestures_enabled)
	{
		...
		if(gesturebuf)
		{
			...
			gesturebuf = FT_REG_GESTURES_OUTPUT;
			...
			data->gestures_blocked = 1;
			init_timer(&ft5x06_gestures_block_timer);
			ft5x06_gestures_block_timer.expires = jiffies + HZ/2;
			ft5x06_gestures_block_timer.function = ft5x06_gestures_unblock;
			ft5x06_gestures_block_timer.data = (unsigned long) data;
			add_timer(&ft5x06_gestures_block_timer);
	...
	for (i = 0; i < data->pdata->num_max_touches; i++) {
		...
		// Firmware reports bigger than 1145 for the "black" bottom area, where the vkeys are, report fake 1320 to Android
		x -= 80;
		x = x * 720 / 640;
...
static ssize_t ft5x06_gestures_enabled_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct ft5x06_ts_data *data = dev_get_drvdata(dev);
...
static ssize_t ft5x06_gestures_enabled_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct ft5x06_ts_data *data = dev_get_drvdata(dev);
	...
	data->gestures_enabled = val;

	return size;
}

static DEVICE_ATTR(gestures_enabled, 0666, ft5x06_gestures_enabled_show, ft5x06_gestures_enabled_store);
...
{
	struct ft5x06_ts_data *data = input_get_drvdata(in_dev);
			   ...
			   const struct i2c_device_id *id)
{
	...
	input_dev->enable = ft5x06_input_enable;
	input_dev->disable = ft5x06_input_disable;
	...
	err = device_create_file(&client->dev, &dev_attr_gestures_enabled);
	if (err) {
		dev_err(&client->dev, "sys file creation failed\n");
		goto free_gestures_enabled_sys;
	}
	...
	{
		data->loading_fw = true;
		ft5x06_fw_upgrade(&data->client->dev, true);
		data->loading_fw = false;
	}
	mutex_unlock(&data->input_dev->mutex);
...
free_gestures_enabled_sys:
	device_remove_file(&client->dev, &dev_attr_gestures_enabled);
